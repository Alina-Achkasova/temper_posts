...
describe('Store', () => {
  ...
  beforeEach(() => {
    ...
    mockStore = createStore({
      state() {
        return {
          posts: [
            { id: 1, userId: 2, title: 'hello1', body: 'world2' },
  ...
  it('should commit mutations and return updated posts and historyItems', () => {
    const newPosts = [
      { id: 0, userId: 1, title: 'hello0', body: 'world1' },
  ...
  it('should add a history item', async () => {
    ...
    expect(store.getters.getHistoryItems).toEqual([
      { postId: 1, oldIndex: 2, newIndex: 3 },
    ]);
  });

  it('should travel back in time', async () => {
    store.commit('SET_HISTORY_ITEMS', [
      { postId: 3, oldIndex: 1, newIndex: 0 },
      { postId: 4, oldIndex: 3, newIndex: 4 },
      { postId: 2, oldIndex: 1, newIndex: 2 },
    ]);
